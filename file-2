function renderHome() {
  const featured = PRODUCTS.slice(0, 4);
  document.getElementById('app').innerHTML =    
    <section class="hero">
      <div class="hero-left">
        <p class="hero-eyebrow">Curated objects for the considered home</p>
        <h1 class="hero-headline">Form<br>without<br><em>excess.</em></h1>
        <p class="hero-sub">Every object made to be used, not displayed. Materials sourced honestly. Production runs kept small.</p>
        <a href="#/catalog" data-link class="hero-cta">
          View catalog
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="5" y1="12" x2="19" y2="12"/><polyline points="12 5 19 12 12 19"/></svg>
        </a>
      </div>
      <div class="hero-right">
        ${featured.map(p => `
  <div class="hero-img-cell" onclick="Router.navigate('/product/${p.id}')" style="cursor:pointer;">
            <div style="display:flex;align-items:center;justify-content:center;width:100%;height:100%;min-height:180px;color:var(--ink);">
              ${p.svg}
            </div>
          </div>
        `).join('')}
      </div>
    </section>
    <section class="section" style="background: var(--surface-2);">
      <div class="section-header">
        <h2 class="section-title">New arrivals</h2>
        <a href="#/catalog" data-link class="section-meta" style="color:var(--accent);">View all →</a>
      </div>
      <div class="product-grid">
        ${PRODUCTS.filter(p => p.badge === 'new').map(renderProductCard).join('')}
      </div>
    </section>
    <!-- Materials strip -->
    <section class="section">
      <div class="section-header">
        <h2 class="section-title">Browse by material</h2>
      </div>
      <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(160px,1fr));gap:2px;">
        ${CATEGORIES.filter(c => c.id !== 'all').map(cat => `
          <button onclick="Router.navigate('/catalog?cat=${cat.id}')" style="padding:1.5rem;background:var(--surface-2);border:none;cursor:pointer;text-align:left;transition:background .2s;font-family:inherit;"
            onmouseover="this.style.background='var(--surface-3)'" onmouseout="this.style.background='var(--surface-2)'">
            <div style="font-size:.75rem;letter-spacing:.12em;text-transform:uppercase;color:var(--ink-faint);margin-bottom:.375rem;font-family:inherit;">${PRODUCTS.filter(p => p.category === cat.id).length} objects</div>
            <div style="font-size:.9375rem;color:var(--ink);font-family:inherit;">${cat.label}</div>
          </button>
        `).join('')}
      </div>
    </section>
  `;
}
function renderProductCard(p) {
  const displayPrice = p.salePrice
    ? `<span class="product-price-original">€${p.price}</span><span class="product-price">€${p.salePrice}</span>`
    : `<span class="product-price">€${p.price}</span>`;
  const badge = p.badge
    ? `<span class="product-badge badge-${p.badge}">${p.badge === 'new' ? 'New' : 'Sale'}</span>`
    : '';
  return `
    <article class="product-card" onclick="Router.navigate('/product/${p.id}')">
      <div class="product-card-img" style="color:var(--ink);">
        ${badge}
        ${p.svg}
      </div>
      <div class="product-card-body">
        <div class="product-material">${p.material}</div>
        <div class="product-name">${p.name}</div>
        <div class="product-price-row">
          <div>${displayPrice}</div>
          <button class="product-quick-add" onclick="event.stopPropagation();Cart.add('${p.id}')">Add</button>
        </div>
      </div>
    </article>
  `;
}

function renderCatalog(activeCat = 'all') {
  const filtered = activeCat === 'all'
    ? PRODUCTS
    : PRODUCTS.filter(p => p.category === activeCat);

  document.getElementById('app').innerHTML = `
   <section class="section">
      <div class="section-header">
        <h2 class="section-title">Catalog</h2>
        <span class="section-meta">${filtered.length} objects</span>
      </div>
      <div class="filters">
        <span class="filter-label">Filter</span>
        ${CATEGORIES.map(cat => `
          <button class="filter-btn ${cat.id === activeCat ? 'active' : ''}"
            onclick="renderCatalog('${cat.id}');history.replaceState(null,'','#/catalog?cat=${cat.id}')">
            ${cat.label}
          </button>
        `).join('')}
      </div>
  <div class="product-grid" id="catalog-grid">
        ${filtered.length
          ? filtered.map(renderProductCard).join('')
          : `<div style="grid-column:1/-1;padding:4rem;text-align:center;color:var(--ink-muted);">No objects in this category yet.</div>`
        }
      </div>
    </section>
  `;
}

function renderProduct(id) {
  const p = PRODUCTS.find(pr => pr.id === id);
  if (!p) { renderNotFound(); return; }

  const displayPrice = p.salePrice
    ? `span style="font-size:1rem;color:var(--ink-faint);text-decoration:line-through;margin-right:.5rem;">€${p.price}</span>€${p.salePrice}`
    : `€${p.price}`;

  document.getElementById('app').innerHTML = `
    <article class="product-detail">
      <div class="product-detail-img" style="color:var(--ink);">
        <div style="transform:scale(2.5);">${p.svg}</div>
      </div>

   <div class="product-detail-body">
        <nav class="breadcrumb">
          <a href="#/" data-link>Home</a>
          <span class="breadcrumb-sep">/</span>
          <a href="#/catalog" data-link>Catalog</a>
          <span class="breadcrumb-sep">/</span>
          <span>${p.name}</span>
        </nav>

   <div class="detail-material">${p.material}</div>
        <h1 class="detail-name">${p.name}</h1>
        <div class="detail-price">${displayPrice}</div>

   <hr class="detail-divider" />
        <p class="detail-desc">${p.description}</p>

  <div class="detail-specs">
          ${Object.entries(p.specs).map(([k, v]) => `
            <div class="detail-spec">
              <span class="detail-spec-label">${k}</span>
              <span class="detail-spec-value">${v}</span>
            </div>
          `).join('')}
        </div>

  <div class="qty-row">
          <span style="font-size:.875rem;color:var(--ink-muted);">Qty</span>
          <div class="qty-control">
            <button class="qty-btn" id="qty-dec" aria-label="Decrease">−</button>
            <input type="number" class="qty-num" id="qty-input" value="1" min="1" max="99" />
            <button class="qty-btn" id="qty-inc" aria-label="Increase">+</button>
          </div>
        </div>

  <button class="add-to-cart" id="atc-btn">Add to cart</button>

   <p style="font-size:.8125rem;color:var(--ink-faint);margin-top:1rem;">
          Free shipping on orders over €200. Returns within 14 days.
        </p>
      </div>
    </article>    <!-- Related -->
    <section class="section" style="background:var(--surface-2);">
      <div class="section-header">
        <h2 class="section-title">More objects</h2>
      </div>
      <div class="product-grid">
        ${PRODUCTS.filter(pr => pr.id !== p.id).slice(0, 4).map(renderProductCard).join('')}
      </div>
    </section>
  `;

  // Qty controls
  const qtyInput = document.getElementById('qty-input');
  document.getElementById('qty-dec').addEventListener('click', () => {
    if (parseInt(qtyInput.value) > 1) qtyInput.value = parseInt(qtyInput.value) - 1;
  });
  document.getElementById('qty-inc').addEventListener('click', () => {
    qtyInput.value = parseInt(qtyInput.value) + 1;
  });

  document.getElementById('atc-btn').addEventListener('click', () => {
    Cart.add(p.id, parseInt(qtyInput.value) || 1);
  });
}

function renderAbout() {
  document.getElementById('app').innerHTML = `
    <div class="about-page">
      <p class="about-eyebrow">About FORMA</p>
      <h1 class="about-headline">Objects made<br>to be <em>lived with</em></h1>

   <div class="about-body">
        <p>FORMA is a curated shop for objects that earn their place in a room. We don't stock novelties. Everything here is made to a material and structural logic that makes it better to use than to admire.</p>
        <p>We work directly with workshops — in Lisbon, Berlin, Kyoto, and Vermont — who share a refusal to separate how something looks from how it works. Production runs are small by design: not as a marketing gesture, but because the makers we trust don't operate at volume.</p>
        <p>Returns are accepted within 14 days if you change your mind. We cannot accept returns of items that have been damaged through use — they were meant to be used.</p>
      </div>
      <div class="about-values">
        <div class="about-value">
          <div class="about-value-num">01</div>
          <div class="about-value-title">Material honesty</div>
          <p class="about-value-desc">Wood looks like wood. Cement looks like cement. We don't stock finishes that disguise the material they cover.</p>
        </div>
        <div class="about-value">
          <div class="about-value-num">02</div>
          <div class="about-value-title">Small production</div>
          <p class="about-value-desc">Every workshop we work with can tell us where each piece was made, and by whom. We keep runs small enough that this stays true.</p>
        </div>
        <div class="about-value">
          <div class="about-value-num">03</div>
          <div class="about-value-title">Objects that age</div>
          <p class="about-value-desc">The best objects in a room are the ones that look better in five years. Patina, wear, and softness over time are features, not damage.</p>
        </div>
        <div class="about-value">
          <div class="about-value-num">04</div>
          <div class="about-value-title">Transparent pricing</div>
          <p class="about-value-desc">We publish what we pay the workshops. Our margin funds the editorial work and covers shipping. No mystery markups.</p>
        </div>
      </div>
      <div class="about-body">
        <p>Questions, stockist enquiries, or workshop proposals: <a href="mailto:studio@forma.co" style="color:var(--accent);text-decoration:underline;">studio@forma.co</a></p>
      </div>
    </div>
  `;
}
function renderNotFound() {
  document.getElementById('app').innerHTML = `
    <div class="not-found">
      <div class="not-found-code">404</div>
      <h2>Page not found</h2>
      <p>The page you're looking for doesn't exist or has moved.</p>
      <a href="#/" data-link class="btn-back">
        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="19 12 5 12"/><polyline points="12 5 5 12 12 19"/></svg>
        Back home
      </a>
    </div>
  `;
}

// ─── ROUTING ───
Router.define('/', renderHome);
Router.define('/catalog', () => {
  const params = new URLSearchParams(window.location.hash.split('?')[1] || '');
  renderCatalog(params.get('cat') || 'all');
});
Router.define('/product/:id', ({ id }) => renderProduct(id));
Router.define('/about', renderAbout);
Router.define('*', renderNotFound);

// ─── INIT ───
document.addEventListener('DOMContentLoaded', () => {
  Cart.init();
  Router.init();
});
